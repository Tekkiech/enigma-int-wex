...
- `server/`: `models.py` (10 SQLAlchemy tables, `schema.dbml` as the diagram source), `app.py` (session-cookie + bcrypt auth, public read-only catalogue routes, `@login_required` cart/wishlist/order routes), `seed.py` (the only file that ever calls DummyJSON)
- `src/stores/catalog.js` and `account.js` (Pinia) mirror the API: cart/wishlist writes are local-first and only sync when signed in, so browsing still needs no account; `POST /api/orders` snapshots the cart into a real order and clears it
- `src/views/`: searchable category tiles on Home, a filterable/sortable grid per category, a product detail page with gallery/specs/reviews, deals, saved items, cart with checkout, account with sign-up/sign-in and order history, and a static Support page (FAQ, delivery, returns)
